...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src: torch.Tensor, src_mask: Optional[torch.Tensor] = None, is_causal = None, src_key_padding_mask: Optional[torch.Tensor] = None) -> torch.Tensor:
...
class MultiHeadAttention(nn.Module):
  def __init__(self, d_model=256, n_head=4, dropout=0.1, relative_positional=True, relative_positional_distance=100, batch_first = True):
    super().__init__()
    
    # Added this to prevent pytorch transformer version issue.
